...
import { generateVitePlugins } from './build/plugins/index';
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        [VITE_ENV.VITE_FILE_DIR]: { target: VITE_ENV.VITE_BASE_URL, changeOrigin: true },
      },
    },
    css: {
      ...
      rollupOptions: {
        output: {
          ...
          assetFileNames(chunkInfo) {
            ...
            if (fontExtList.some(ext => name.endsWith(ext))) {
              return 'font/[name]-[hash].[ext]';
            }
            // 图片文件
            const imgExtList = ['png', 'jpg', '.jpeg', 'gif', 'webp'];
            if (imgExtList.some(ext => name.endsWith(ext))) {
              return `img/[name]-[hash].[ext]`;
            }
            // 视频文件
            const videoExtList = ['.mp4', '.avi', '.wmv', '.ram', '.mpg', 'mpeg', '.m3u8'];
            if (videoExtList.some(ext => name.endsWith(ext))) {
