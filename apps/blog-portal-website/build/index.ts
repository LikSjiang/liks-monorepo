 ...
 * @Description: 处理环境变量
 * @Author: liks
 * @Date: 2025-10-29 15:02:40
 * @LastEditors: liks
 * @LastEditTime: 2025-10-29 15:06:37
 */
...
export function warpperEnv(env_config: Recordable<string>): ViteEnv {
  ...
  for (const [key, value] of Object.entries(env_config)) {
    if (['true', 'false'].includes(value)) {
      (env as any)[key] = value === 'true' ? true : false; // 布尔类型处理
    } else if (Number(value)) {
      (env as any)[key] = Number(value); // 数值类型处理
    } else {
      (env as any)[key] = value; // 字符串类型原值即可
    }
  }
  return env;
}
